...
export default function SearchResultGuarantee() {
  ...
  const checkSection = (
    prevSection: ("popular" | "recommended")[] = [],
    section: "popular" | "recommended",
  ) => {
    return prevSection.includes(section)
      ? prevSection.filter((s) => s !== section)
      : [...prevSection, section];
  };
  ...
  const handleEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (!mobileSearch.trim()) {
        router.push(`/private-trip`);
        searchRef.current?.blur();
        // setInputSearchFocus(false);
        return;
      }

      router.push(`/private-trip?q=${encodeURIComponent(mobileSearch)}`);
      searchRef.current?.blur();
  ...
  const updateQueryParams = () => {
    ...
    setOrDelete(
      "section",
      filters.section?.length ? filters.section.join(",") : null,
    );
    ...
    setOrDelete(
      "page",
      filters.page && filters.page !== 1 ? String(filters.page) : null,
    );
  ...
  useEffect(() => {
    ...
    setMobileSearch(q);
    // SECTION
    const rawSections = searchParams.get("section")
      ? searchParams.get("section")!.split(",")
      : [];
    const allowedSections = ["popular", "recommended"] as const;
    const section = rawSections.filter((s): s is "popular" | "recommended" =>
      allowedSections.includes(s as any),
    );
    // END SECTION
    ...
    setFilters({ q, section, days, category, minPrice, maxPrice, page });
                ...
                <div className="px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="relative flex items-center flex-1">
                      ...
                      <input
                        type="text"
                        placeholder="Cari trip pasti jalan"
                        value={mobileSearch}
